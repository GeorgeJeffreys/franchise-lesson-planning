 ...
 * admin-authored steering text that shapes the objective checker (Aya,
 * POST /api/check-objective). Accepts a plain-text `.md` or `.txt` file, or a
 * Word `.docx`, as multipart/form-data (`file` field). Text files are read
 * as-is; a `.docx` is converted to markdown server-side (mammoth → HTML →
 * turndown, images dropped). Either way the resulting text is INSERTed as a new
 * `smartt_objective_guide` row (`uploaded_by` = the caller). Each upload is a new
 * immutable version; the latest is active.
 *
 * Auth: a signed-in admin only. Non-admins get 403; the table's RLS
 * (`smartt_objective_guide_insert_admin`) is the backstop.
 *
 * This route is a faithful clone of `/api/ai-resource-guide` — see that route
