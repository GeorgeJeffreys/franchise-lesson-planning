 ...
 *  A sibling of {@link ResourceGuideVersion} — same shape, separate stack. */
 ...
 * A faithful clone of {@link getActiveResourceGuideVersion}. Returns null when no
 * guide has been uploaded yet (the UI then notes that the built-in default guide
 * is in use). Admin-only by RLS (`smartt_objective_guide_select_admin`); a
 * non-admin read yields no rows → null.
 *
 * The uploader's name is best-effort: there is no admin-wide `profiles` SELECT
 * policy (see the RLS note at the top of this file), so it resolves only when the
 * admin shares a space with the uploader (commonly themselves). Falls back to null.
