...
/**
 * A light "view only" banner shown above the content on the plan pane while the
 * plan is locked (`submitted` / `approved`). It tells the teacher why editing is
 * disabled and routes them to the Review step, where the Submit/Recall control is
 * the one way to toggle the lock. The author can now recall an `approved` plan
 * too, so the link is shown in both locked states; only the title copy differs.
 */
