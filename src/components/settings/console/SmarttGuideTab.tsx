...
/**
 * Admin "SMARTT objective guide" sub-section. Shows the active guide version
 * (when / by whom), an upload control (.md / .txt read verbatim, or .docx
 * converted to markdown), and a read-only preview of the current guide text.
 * Upload-only — there is no inline editing. Each upload POSTs a new immutable
 * version to `/api/smartt-objective-guide`, then refreshes.
 *
 * When no version exists, the checker uses a built-in default guide; the UI says
 * so and the preview is omitted.
 *
 * Faithful clone of {@link AiGuideTab} — different endpoint and copy.
 */
export function SmarttGuideTab({ active }: { active: SmarttGuideVersion | null }) {
  ...
  function onFile(file: File) {
    ...
    startTransition(async () => {
      try {
        const res = await fetch('/api/smartt-objective-guide', { method: 'POST', body: fd });
        if (!res.ok) {
          const data = (await res.json().catch(() => null)) as { error?: string } | null;
          setError(data?.error ?? 'Upload failed. Please try again.');
          return;
        }
        setMessage('New guide version saved. It is now active for all objective checks.');
        router.refresh();
      } catch {
